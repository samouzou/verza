...
export default function PostGigPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user || !user.primaryAgencyId) {
      toast({
        ...
        description: "You must be associated with an agency to launch a campaign.",
        variant: 'destructive'
      });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Redirecting to Payment", description: "Please complete the funding to launch your campaign." });
      ...
      toast({
        ...
        description: error.message || 'Could not launch the campaign.',
        variant: 'destructive'
      });
  ...
  if (!user || !canPostGigRole) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-4">
        ...
        <p className="text-muted-foreground">Only agency team members can launch new campaigns.</p>
      </div>
    );
  }
  ...
  if (!canPost) {
    return (
      <>
        <PageHeader title="Launch a New Campaign" description="Recruit creators for your next campaign." />
  ...
  return (
    <>
      <PageHeader
        ...
        description="Describe your campaign, set the rate, and recruit creators."
        actions={
          <Button variant="outline" asChild>
            <Link href="/campaigns"><ArrowLeft className="mr-2 h-4 w-4" /> Cancel</Link>
          </Button>
        }
      />
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  {(trackingMethod === 'promo_code_only' || trackingMethod === 'both') && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-blue-500/10">
                      <div className="space-y-2">
                        ...
                        <Input id="promoCodeSuffix" value={promoCodeSuffix} onChange={e => setPromoCodeSuffix(e.target.value.toUpperCase())} placeholder="e.g. 30 or VERZA" disabled={isSubmitting} required={isAffiliateEnabled} />
            ...
            {campaignType === 'cause_campaign' ? (
              <Card className="shadow-lg border-rose-500/20 bg-rose-50/5">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2"><Heart className="h-5 w-5 text-rose-500" /> 4. Compensation</CardTitle>
            ...
            <Card className="shadow-lg border-primary/10">
              <CardHeader>
                ...
                <CardDescription>Define how you plan to use the content created for this campaign.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                      ...
                      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
                        ...
                        <ScrollArea className="flex-1 mt-4 pr-4 border rounded-md p-4 bg-muted/10">
                          <div className="space-y-4 text-sm leading-relaxed">
                            ...
                            <p className="font-bold">2. CAMPAIGN NETWORK ROLE</p>
                            <p>
              ...
              {totalAmount > 0 ? (
                ...
                <div className="p-6 border rounded-lg bg-blue-500/5 text-center border-blue-500/20">
                  <p className="text-sm text-blue-600 font-bold flex items-center justify-center gap-2 uppercase tracking-widest">
                    <Zap className="h-4 w-4" /> Pure Performance Campaign
                  </p>
