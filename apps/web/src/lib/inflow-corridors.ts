...
/** Resolve ISO-2 or Inflow codes to an Inflow corridor code. */
...
/** Pick a unique ISO-2 value for the country selector from stored/API input. */
export function iso2ForInflowCountryInput(code: string | null | undefined): string {
  if (!code) return "NG";
  const upper = code.trim().toUpperCase();
  const byIso2 = INFLOW_COUNTRY_OPTIONS.find((option) => option.iso2 === upper);
  if (byIso2) return byIso2.iso2;
  const inflowCode = toInflowCountryCode(upper);
  if (!inflowCode) return "NG";
  return INFLOW_COUNTRY_OPTIONS.find((option) => option.inflowCode === inflowCode)?.iso2 ?? "NG";
}
