...
export type AgencyOpticBillingLite = {
  plan: string | null;
...
function chargeDocId(jobId: string, profileUrl: string): string {
  const hash = createHash("sha256").update(`${jobId}|${profileUrl}`).digest("hex").slice(0, 32);
  return `${jobId}_${hash}`;
}

/** Same credit semantics as functions saveLeadWithOpticCreditCharge. */
export async function saveLeadWithOpticCreditCharge(params: {
  db: Firestore;
  jobId: string;
  agencyId: string;
  profileUrl: string;
  leadData: Record<string, unknown>;
  billing: AgencyOpticBillingLite;
}): Promise<SaveLeadResult> {
  const {db, jobId, agencyId, profileUrl, leadData, billing} = params;
  const chargeRef = db.collection("optic_credit_charges").doc(chargeDocId(jobId, profileUrl));
  ...
  if (result === null) return {ok: false, reason: "needs_top_up"};
  if (result === undefined) return {ok: false, reason: "insufficient_credits"};
  return {
    ok: true,
...
export function billingFromActor(actor: VerzaActor): AgencyOpticBillingLite {
  return {
    plan: actor.opticPlan,
    subscriptionActive: actor.opticSubscriptionActive,
    balance: actor.opticCreditsBalance,
  };
}
