...
import type {Contract} from "../../../src/types"; // Ensure this path is correct relative to the functions dir
...
export const helloSignWebhookHandler = onRequest(async (request, response) => {
  ...
  if (actualEventPayload && typeof actualEventPayload === 'object' && actualEventPayload.event && actualEventPayload.event.event_type === "test") {
    logger.info("Received Dropbox Sign test event. Responding with 200 OK.", actualEventPayload);
  ...
  if (!actualEventPayload || typeof actualEventPayload !== 'object') {
    logger.error("Webhook payload could not be successfully parsed into a usable object or was empty/unrecognized.", {
      ...
      bodyPreview: Buffer.isBuffer(request.body) ? request.body.toString('utf8', 0, 250) : (typeof request.body === 'string' ? request.body.substring(0,250) : 'Non-string/buffer body'),
    });
  ...
  try {
    ...
    if (eventType.startsWith("signature_request_") && !signatureRequestId) {
        ...
        response.status(200).send("Hello API Event Received (Error Logged)"); 
        return;
    }
