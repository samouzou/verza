...
export function BrandDeckPreview({ guide, agencyName, products = [] }: BrandDeckPreviewProps) {
  ...
  const renderSlides = () => {
    ...
    // Slide 1: Intro
    allSlides.push(
      <div key="intro" className="h-full w-full flex flex-col items-center justify-center p-12 text-center space-y-6 relative overflow-hidden">
        <div className="absolute inset-0 opacity-[0.08]" style={{ background: `linear-gradient(135deg, ${primaryColor}, ${secondaryColor})` }} />
        <div className="relative z-20 flex flex-col items-center space-y-6 w-full">
          {logoUrl ? <img src={logoUrl} alt={agencyName} className="h-24 w-auto object-contain drop-shadow-xl" /> : (
            <div className="h-24 w-24 rounded-2xl bg-primary/10 flex items-center justify-center text-primary text-4xl font-bold shadow-sm">{agencyName.charAt(0)}</div>
          )}
    ...
    // Slide 2: Visuals
    allSlides.push(
      <div key="visuals" className="h-full w-full p-8 flex flex-col bg-white relative">
        ...
        <div className="grid grid-cols-2 gap-4">
           <div className="space-y-3">
              ...
              <div className="h-20 rounded-xl shadow-inner border flex items-end p-3" style={{ backgroundColor: primaryColor }}>
                 <span className="text-[10px] font-mono bg-white/60 px-1.5 py-0.5 rounded text-black font-bold border border-black/10">{primaryColor}</span>
              </div>
           </div>
           <div className="space-y-3">
              ...
              <div className="h-20 rounded-xl shadow-inner border flex items-end p-3" style={{ backgroundColor: secondaryColor }}>
                 <span className="text-[10px] font-mono bg-white/60 px-1.5 py-0.5 rounded text-black font-bold border border-black/10">{secondaryColor}</span>
              </div>
           </div>
        </div>
        <div className="space-y-4 pt-8 mt-8 border-t border-dashed flex-1 flex flex-col">
           <div className="flex items-center gap-2 text-muted-foreground font-medium text-xs uppercase"><Type className="h-3 w-3" /> Typography</div>
           <div className="p-8 rounded-xl bg-muted/30 border shadow-sm flex-1 flex flex-col justify-center">
              <p className="text-3xl leading-tight text-foreground" style={{ fontFamily: typography || 'inherit' }}>The quick brown fox jumps over the lazy dog.</p>
    ...
    // Slide 3: Voice
    allSlides.push(
      <div key="voice" className="h-full w-full p-8 flex flex-col bg-white relative">
        ...
        <div className="space-y-3 mb-8">
           ...
           <p className="text-xl leading-relaxed italic text-foreground/80">"{toneOfVoice || "Our voice is still being defined. We value authenticity and clarity."}"</p>
        </div>
        <div className="grid grid-cols-2 gap-6 pt-8 border-t border-dashed flex-1">
           <div className="space-y-4">
              <div className="flex items-center gap-1.5 text-green-600 font-bold text-[10px] uppercase tracking-wider"><CheckCircle2 className="h-4 w-4" /> Dos</div>
              <div className="space-y-2">
                 {dos.filter(d => !!d).length > 0 ? dos.filter(d => !!d).slice(0, 4).map((doItem, i) => (
                    <div key={i} className="text-xs p-3 rounded-lg bg-green-50/50 border border-green-100 text-green-800 shadow-sm">{doItem}</div>
                 )) : <p className="text-[10px] text-muted-foreground italic">No specific dos yet.</p>}
              </div>
           </div>
           <div className="space-y-4">
              <div className="flex items-center gap-1.5 text-red-600 font-bold text-[10px] uppercase tracking-wider"><XCircle className="h-4 w-4" /> Don'ts</div>
              <div className="space-y-2">
                 {donts.filter(d => !!d).length > 0 ? donts.filter(d => !!d).slice(0, 4).map((dontItem, i) => (
                    <div key={i} className="text-xs p-3 rounded-lg bg-red-50/50 border border-red-100 text-red-800 shadow-sm">{dontItem}</div>
    ...
    if (productChunks.length === 0) {
      allSlides.push(
        <div key="products-empty" className="h-full w-full p-8 flex flex-col bg-white relative">
           ...
           <div className="flex-1 flex flex-col items-center justify-center text-center space-y-3">
             <div className="p-6 rounded-full bg-muted/50 text-muted-foreground/30"><ShoppingBag className="h-12 w-12" /></div>
      ...
      productChunks.forEach((chunk, chunkIndex) => {
        allSlides.push(
          <div key={`products-${chunkIndex}`} className="h-full w-full p-8 flex flex-col bg-white relative">
            <div className="flex items-center justify-between mb-6 shrink-0">
               <div className="flex items-center gap-2 text-primary font-semibold text-sm uppercase tracking-wider">
                  <ShoppingBag className="h-4 w-4" />
                  Lookbook {productChunks.length > 1 ? `(${chunkIndex + 1}/${productChunks.length})` : ''}
               </div>
               <span className="text-[10px] text-muted-foreground font-medium uppercase tracking-widest">Select product for spotlight</span>
            </div>
            <div className="grid grid-cols-2 grid-rows-2 gap-3 flex-1 min-h-0">
               {chunk.map((product: BrandProduct) => (
                 <div 
                   key={product.id} 
                   className="group relative rounded-xl border bg-muted/20 overflow-hidden flex flex-col shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer min-h-0"
                   onClick={() => setSelectedProduct(product)}
                 >
                    <div className="flex-1 min-h-0 w-full bg-white relative overflow-hidden">
                       {product.imageUrl ? (
                       ...
                       <div className="absolute inset-0 bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                          <div className="bg-white/90 backdrop-blur-sm px-2.5 py-1 rounded-full shadow-lg border border-primary/10 scale-90 group-hover:scale-100 transition-transform">
                    ...
                    <div className="p-2 space-y-0.5 shrink-0 bg-white/50 backdrop-blur-sm border-t">
    ...
    if (bRollLibrary.length > 0) {
      allSlides.push(
        <div key="b-roll" className="h-full w-full p-8 flex flex-col bg-white relative">
          ...
          <div className="grid grid-cols-2 gap-4 overflow-y-auto pr-2 scrollbar-thin">
             {bRollLibrary.map((asset) => (
               <div key={asset.id} className="group relative rounded-xl border bg-black overflow-hidden aspect-video flex items-center justify-center shadow-md">
                  <video src={asset.url} className="h-full w-full object-cover opacity-60" />
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                     <Button variant="secondary" size="sm" className="bg-white/90 hover:bg-white text-primary font-bold text-[10px]" onClick={(e) => { e.stopPropagation(); handleDownload(asset.url, `broll_${asset.id}.mp4`); }}>
                       <Download className="h-3 w-3 mr-1.5" /> Download Clip
                     </Button>
                  </div>
                  <div className="absolute bottom-2 left-2 right-2"><p className="text-[9px] text-white/70 font-mono truncate">{asset.name}</p></div>
               </div>
             ))}
          </div>
        </div>
  ...
  return (
    <div className="w-full h-full min-h-[400px] flex flex-col bg-white rounded-2xl shadow-2xl overflow-hidden border border-primary/10 select-none relative" onWheel={handleWheel}>
      {/* Top Bar */}
      <div className="h-12 border-b bg-muted/20 flex items-center justify-between px-6 shrink-0 z-30">
         <div className="flex gap-1.5">
            <div className="w-2.5 h-2.5 rounded-full bg-red-400" />
            <div className="w-2.5 h-2.5 rounded-full bg-amber-400" />
            <div className="w-2.5 h-2.5 rounded-full bg-emerald-400" />
         </div>
         <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.3em]">{agencyName} identity</div>
         <div className="text-[10px] font-mono text-muted-foreground/40">Slide {currentSlide + 1} / {slidesCount}</div>
      </div>
      ...
      <div className="flex-1 min-h-0 relative overflow-hidden bg-white isolate">
         ...
         {selectedProduct && (
           <div className="absolute inset-0 z-50 bg-white animate-in fade-in slide-in-from-bottom-4 duration-300 flex flex-col">
              <div className="flex items-center justify-between p-6 border-b shrink-0">
                 <div className="flex items-center gap-3">
                    <div className="bg-primary/10 p-2 rounded-lg"><ShoppingBag className="h-5 w-5 text-primary" /></div>
                    <h3 className="text-xl font-bold tracking-tight uppercase">{selectedProduct.name}</h3>
                 </div>
                 <Button variant="ghost" size="icon" onClick={() => setSelectedProduct(null)} className="rounded-full h-10 w-10"><X className="h-6 w-6" /></Button>
              </div>
              ...
              <div className="flex-1 min-h-0 flex overflow-hidden">
                 ...
                 {/* Product Info View */}
                 <div className="w-[320px] shrink-0 p-8 border-l flex flex-col justify-between bg-white overflow-y-auto">
                    <div className="space-y-8">
                       <div className="space-y-2">
                          <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">About the product</p>
                          <p className="text-sm leading-relaxed text-foreground/80">{selectedProduct.description}</p>
                       </div>

                       <div className="space-y-4">
                          <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Unique Selling Points</p>
                          <div className="flex flex-wrap gap-2">
                             {selectedProduct.usps?.map((usp, i) => (
                               <div key={i} className="bg-primary/5 text-primary border border-primary/10 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2">
      ...
      {/* Bottom Footer Info */}
      <div className="h-10 border-t bg-muted/5 flex items-center justify-between px-6 shrink-0 z-30">
         <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-green-500" />
            <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Live Deck</span>
         </div>
         <div className="text-[9px] text-muted-foreground italic px-8 truncate">
